...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex w-full max-w-4xl flex-col gap-6 p-4">
            <Heading
                title="Profit & Loss"
                description="Laba rugi dari Revenue dan Expense pada periode tertentu."
            />

            <div class="rounded-lg border border-sidebar-border/70 p-4">
                <form class="flex flex-wrap items-end gap-2" @submit.prevent="submitFilter">
                    <div class="grid gap-1">
                        <Label for="date_from" class="text-xs text-muted-foreground">From</Label>
                        <Input id="date_from" v-model="filterForm.date_from" type="date" class="w-44" />
                    </div>
                    <div class="grid gap-1">
                        <Label for="date_to" class="text-xs text-muted-foreground">To</Label>
            ...
            <div class="rounded-lg border border-sidebar-border/70 p-4">
                <h3 class="mb-3 text-sm font-semibold">Revenue</h3>
                <table class="w-full text-sm">
                    <tbody>
                        <tr v-if="props.revenue.length === 0">
                            <td colspan="2" class="py-3 text-center text-muted-foreground">Tidak ada akun Revenue dengan transaksi di periode ini.</td>
                        </tr>
                    ...
                    <tfoot>
                        <tr class="border-t border-sidebar-border/70 font-semibold">
                            <td class="py-2 pr-3">Total Revenue</td>
            ...
            <div class="rounded-lg border border-sidebar-border/70 p-4">
                <h3 class="mb-3 text-sm font-semibold">Expense</h3>
                <table class="w-full text-sm">
                    <tbody>
                        <tr v-if="props.expense.length === 0">
                            <td colspan="2" class="py-3 text-center text-muted-foreground">Tidak ada akun Expense dengan transaksi di periode ini.</td>
                        </tr>
                    ...
                    <tfoot>
                        <tr class="border-t border-sidebar-border/70 font-semibold">
                            <td class="py-2 pr-3">Total Expense</td>
            ...
            >
                <div class="flex items-center justify-between text-sm font-semibold" :class="isNetIncomePositive() ? 'text-green-700' : 'text-red-700'">
                    <span>Net Income (Revenue - Expense)</span>
