...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Record Payment - ${props.invoice.invoice_number}`, href: `/sales/invoices/${props.invoice.id}/record-payment` },
];
...
const submit = () => {
    form.post(`/sales/invoices/${props.invoice.id}/record-payment`);
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex w-full max-w-lg flex-col gap-6 p-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <Heading title="Record Payment" description="Catat pembayaran yang diterima untuk invoice ini." />
            ...
            <div class="rounded-lg border border-sidebar-border/70 p-4">
                <div class="mb-5 grid gap-2 rounded-md bg-muted/40 px-3 py-2 text-sm">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-mono">{{ Number(invoice.total_amount).toLocaleString() }} {{ invoice.currency_code }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-muted-foreground">Already Paid</span>
                        <span class="font-mono">{{ Number(invoice.amount_paid).toLocaleString() }} {{ invoice.currency_code }}</span>
                    </div>
                    <div class="flex items-center justify-between border-t border-sidebar-border/70 pt-2 font-semibold">
                        <span>Balance Due</span>
                        <span class="font-mono">{{ Number(invoice.balance_due).toLocaleString() }} {{ invoice.currency_code }}</span>
                    </div>
                </div>
